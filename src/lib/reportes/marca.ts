/**
 * Piezas compartidas por todo lo que la aplicación imprime o exporta.
 *
 * El dashboard autónomo y el reporte de documentos son dos archivos distintos,
 * pero salen de la misma marca: si la paleta viviera duplicada en cada uno,
 * tarde o temprano un color cambiaría en `globals.css` y solo uno de los dos se
 * enteraría.
 */

/**
 * Los mismos tokens de `globals.css`, en claro. Cada documento exportado los
 * declara en su propio `:root`, así que las cadenas `var(--x)` que vienen del
 * modelo pintan igual que en la aplicación y no hay dos paletas que mantener
 * sincronizadas —solo esta lista.
 */
export const TOKENS = `
  --background: #f8faff;
  --foreground: #1e2e5a;
...
export function fechaLarga(momento: Date): string {
...
/** `2026-09-11`, para los nombres de archivo. */
export function selloFecha(momento: Date): string {
